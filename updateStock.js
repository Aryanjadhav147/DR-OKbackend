...
const addStockField = async () => {
  try {
    ...
    if (!inventoryDb) {
      ...
      console.error(
        "Make sure you have 'inventoryKey.json' in your server folder and 'config/firebase.js' is updated.",
      );
      return;
    }
    ...
    if (snapshot.empty) {
      ...
      console.log(
        "Check if your collection name is exactly 'medicines' (case sensitive).",
      );
      return;
    }
    ...
    await batch.commit();
    console.log(
      `✅ Success! Added 'stock: 100' to ${count} medicines in Inventory DB!`,
    );
  } catch (error) {
